...
import { billingRouter, handlePaystackWebhook, handlePaddleWebhook, requireActiveSubscription, getEntitlement, paywallEnabled, billingConfigured, setupBillingSchema, loadPlanPricingOverrides } from './billing.js'
...
const RATE_EXEMPT = new Set(['/health', '/v1/test-llm', '/v1/test-cerebras', '/v1/feedback', '/v1/cache-stats', '/v1/test-search', '/v1/billing/webhook', '/v1/billing/webhook/paystack', '/v1/billing/webhook/paddle'])
...
// ── Accounts & billing ────────────────────────────────────────────────────────
// Provider webhooks FIRST — they need the raw body (captured above) and must
// never be behind auth. /v1/billing/webhook is kept as an alias to Paystack's
// handler for backward compatibility with webhook URLs configured before Paddle
// support existed. Then attach the signed-in user to every request, and mount
// the auth + billing routers.
app.post('/v1/billing/webhook', handlePaystackWebhook)
app.post('/v1/billing/webhook/paystack', handlePaystackWebhook)
app.post('/v1/billing/webhook/paddle', handlePaddleWebhook)
app.use(attachUser)
...
app.post('/v1/setup', async (req, res) => {
  ...
  try {
    await setupSchema()
    await setupAuthSchema()
    await setupBillingSchema()
    await loadPlanPricingOverrides()
    await seedKnowledge()
    res.json({ ok: true, message: 'NORIA schema created (knowledge + accounts + subscriptions + pricing) and seed knowledge ingested.' })
  } catch (e) {
...
app.get('/v1/setup', async (req, res) => {
  ...
  try {
    ...
    await loadPlanPricingOverrides()
    await seedKnowledge()
    res.send('✅ NORIA setup complete — knowledge, accounts, subscription and pricing tables created; all knowledge (Africa + SkyGlobe) ingested. You can close this tab.')
  } catch (e) {
...
app.listen(PORT, () => {
  ...
  if (process.env.DATABASE_URL) {
    setupAuthSchema()
      ...
      .then(() => setupBillingSchema())
      .then(() => loadPlanPricingOverrides())
      .catch((e) => console.error('auth/billing schema init failed (accounts/pricing may not work until /v1/setup is run):', e.message))
  }
})
